<?xml version="1.0"?>
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:axf="http://www.antennahouse.com/names/XSL/Extensions" version="2.0">
  <!-- ============================================================== -->
	<xsl:variable name="compare.report.location">
		<xsl:call-template name="getJobTicketParam">
		<xsl:with-param name="varname">compare-report</xsl:with-param>
		</xsl:call-template>
	</xsl:variable>

  <xsl:param name="compare.report.document" select="document(concat('file:///', $compare.report.location),/)" />
	...
	<xsl:template name="get.comparison.result.id">
		...
		<xsl:if test="$compare-available='yes'">
			<xsl:variable name="topic.id">
				<xsl:value-of select="$node/ancestor-or-self::node()[contains(@class,' topic/topic ')][1]/@oid"/>
			</xsl:variable>		
			<xsl:choose>
				<xsl:when test="$compare.report.document//comparereport[@language=$current-language]/object[@ishref=$topic.id]">
					<xsl:value-of select="$compare.report.document//comparereport[@language=$current-language]/object[@ishref=$topic.id]/@compareresult" />
				</xsl:when>
				<xsl:otherwise>
					<!-- in case of topics inside a ditabase or topic in topic we might end up here as the id found will be the one of the 
					     topic, so not the one of the root which is the one present in the compare report
					-->
